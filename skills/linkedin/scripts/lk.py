...
def get_api():
    ...
    if not li_at or not jsessionid:
        print(
            "Error: LINKEDIN_LI_AT and LINKEDIN_JSESSIONID environment variables not set."
        )
        sys.exit(1)
...
def whoami(api):
    ...
    headline = profile.get(
        "headline", profile.get("miniProfile", {}).get("occupation", "No headline")
    )
...
def main():
    ...
    feed_parser.add_argument(
        "-n", "--count", type=int, default=10, help="Number of posts to fetch"
    )
