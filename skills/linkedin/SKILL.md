...
## Browser Tool (Minimal Brave Automation)

The backend exposes a `browser` tool backed by Brave + Playwright. Supported actions:

- **Navigate:** `browser action=navigate targetUrl="https://www.linkedin.com"`
- **Snapshot:** `browser action=snapshot`
- **Act:** `browser action=act kind=click selector="CSS_OR_PLAYWRIGHT_SELECTOR"`
- **Type:** `browser action=act kind=type selector="CSS_SELECTOR" input="Your text" submit=true`
