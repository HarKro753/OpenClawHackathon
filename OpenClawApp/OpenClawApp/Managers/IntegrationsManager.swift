...
final class IntegrationsManager {
    ...
    func saveNotionKey() async {
        ...
        var request = URLRequest(
            url: baseURL.appendingPathComponent("/api/integrations/notion")
        )
    ...
    func saveLinkedinCookies() async {
        ...
        let jsessionId = linkedinJsessionId.trimmingCharacters(
        ...
        var request = URLRequest(
            url: baseURL.appendingPathComponent("/api/integrations/linkedin")
        )
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try? JSONSerialization.data(withJSONObject: [
        ...
        do {
            ...
            if (200..<300).contains(httpResponse.statusCode) {
                ...
                linkedinMessage =
                    error?["error"] as? String
                    ?? "Failed to save LinkedIn cookies."
            }
        } catch {
